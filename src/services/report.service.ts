...
const REQUIRED_BOTH = Array.from(
  new Set([...REQUIRED_DECK, ...REQUIRED_ENGINE]),
);
...
export async function buildReport(sessionId: string) {
  ...
  const validationProfile = latestValidation?.holderProfile as Record<
    string,
    unknown
  > | null;
  ...
  const holderSummary = {
    fullName:
      validationProfile?.["fullName"] ?? holderSource?.holderName ?? null,
    dateOfBirth:
      validationProfile?.["dateOfBirth"] ?? holderSource?.dateOfBirth ?? null,
    nationality: validationProfile?.["nationality"] ?? null,
    sirbNumber:
      validationProfile?.["sirbNumber"] ?? holderSource?.sirbNumber ?? null,
    passportNumber:
      validationProfile?.["passportNumber"] ??
  ...
  const documentChecklist = requiredTypes.map((docType) => {
    ...
    const validity = row?.validity as {
      dateOfExpiry?: string;
      dateOfIssue?: string;
    } | null;
    ...
    return {
      ...
      daysUntilExpiry: validity?.dateOfExpiry
        ? ((row?.validity as Record<string, unknown> | null)?.[
            "daysUntilExpiry"
          ] ?? null)
        : null,
      flags,
  ...
  const medicalData = medicalRow?.medicalData as Record<string, unknown> | null;
  const pemeValidity = peme?.validity as { dateOfExpiry?: string } | null;

  const medicalSummary = {
    fitnessResult: medicalData?.["fitnessResult"] ?? "N/A",
    drugTestResult:
      (drug?.medicalData as Record<string, unknown> | null)?.[
        "drugTestResult"
      ] ?? "N/A",
  ...
  if (Array.isArray(validationMedicalFlags)) {
    for (const flag of validationMedicalFlags as Array<{
      severity: string;
      message: string;
    }>) {
      complianceIssues.push({
  ...
  const expiringDocuments = extractionRows
    .filter((row) => {
      ...
      return (
        validity?.daysUntilExpiry !== undefined &&
