...
export async function extractRoute(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      const mode = (request.query as Record<string, string>)["mode"] ?? "sync";
      ...
      if (!data) {
        throw new AppError(
          "UNSUPPORTED_FORMAT",
          'No file was uploaded. Send a multipart/form-data request with a "document" field.',
        );
      }
...
function formatExtractionResponse(
