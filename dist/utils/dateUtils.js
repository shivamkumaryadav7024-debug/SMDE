...
export function parseDate(raw) {
    ...
    if (NON_EXPIRY_VALUES.has(trimmed.toLowerCase()))
        return null;
    // DD/MM/YYYY
...
export function daysUntilExpiry(raw) {
    if (!raw)
        return null;
    if (NON_EXPIRY_VALUES.has(raw.trim().toLowerCase()))
        return null;
...
export function isExpired(raw) {
    ...
    if (days === null)
        return false; // no expiry = never expired
    return days < 0;
}
/**
 * Returns true if the document expires within the given number of days.
 */
export function isExpiringSoon(raw, withinDays = 90) {
    const days = daysUntilExpiry(raw);
    if (days === null)
        return false;
