...
export async function enqueueExtractionJob(payload) {
    const pendingCount = await db.$count(jobs, eq(jobs.status, "QUEUED"));
    const queuePosition = pendingCount + 1;
    console.log(`[queue] Enqueuing job for file: ${payload.fileName}, queue position: ${queuePosition}`);
    ...
    const pgBossId = await getBoss().send(QUEUE_NAME, { ...payload, jobId }, {
        expireInHours: 1,
        retryLimit: 1,
        retryDelay: 5,
    });
    if (!pgBossId) {
        throw new Error(`pg-boss rejected job for ${jobId} — queue may not exist or send was throttled`);
    }
    console.log(`[queue] ✅ Job sent to pg-boss: jobId=${jobId} pgBossId=${pgBossId}`);
    return jobId;
}
